...
Stdio.prototype.on = function on(event, listener) {
...
Stdio.prototype.emit = function emit(event, ...args) {
  if (this.listeners[event]) {
...
ChildProcess.prototype.on = function on(event, listener) {
  if (!this.listeners[event]) {
    this.listeners[event] = [];
  }
  this.listeners[event].push(listener);
...
ChildProcess.prototype.emit = function emit(event, ...args) {
  if (this.listeners[event]) {
    this.listeners[event].forEach((listener) => listener(...args));
  }
};
